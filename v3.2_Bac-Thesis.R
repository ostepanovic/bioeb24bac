...
#   └──────────────────────────────────────────────────────────────────────────────┘
data <- data[ , !(names(data) %in% c("ID", "N_Days"))]
#   ┌──────────────────────────────────────────────────────────────────────────────┐
...
#   ┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛
index <- createDataPartition(data$Stage, p=0.7, list=FALSE)
trainData <- data[index, ]
testData <- data[-index, ]
testData$Stage <- factor(testData$Stage, levels=levels(trainData$Stage))
#    ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─
#   ┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓
#   ┃                 5. Model training (neural network with nnet)                 ┃
#   ┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛
nnet_model <- nnet(Stage ~ ., data=trainData, size=10, decay=0.1, maxit=300)
nnet_predictions <- predict(nnet_model, testData, type="class")
nnet_predictions <- factor(nnet_predictions, levels=levels(testData$Stage))
#   ┌──────────────────────────────────────────────────────────────────────────────┐
#   │                    5.1. Model evaluation (neural network)                    │
#   └──────────────────────────────────────────────────────────────────────────────┘
confusionMatrix(nnet_predictions, testData$Stage)
cm <- confusionMatrix(nnet_predictions, testData$Stage)
...
nnet_model_optimized <- caret::train(Stage ~ ., data = trainData, method = "nnet",
...
confusionMatrix(nnet_predictions_optimized, testData$Stage)
cm <- confusionMatrix(nnet_predictions_optimized, testData$Stage)
cm_table <- as.table(cm$table)
accuracy <- sum(diag(cm$table)) / sum(cm$table)
ggplot(data = as.data.frame(cm$table), aes(x = Reference, y = Prediction, fill = Freq)) +
  geom_tile() + 
...
#   ┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛
rf_model <- randomForest(Stage ~ ., data=trainData, ntree = 300)
rf_predictions <- predict(rf_model, testData)
#   ┌──────────────────────────────────────────────────────────────────────────────┐
#   │                    6.1. Model evaluation (random forest)                     │
#   └──────────────────────────────────────────────────────────────────────────────┘
confusionMatrix(rf_predictions, testData$Stage)
cm <- confusionMatrix(rf_predictions, testData$Stage)
cm_table <- as.table(cm$table)
accuracy <- sum(diag(cm$table)) / sum(cm$table)
ggplot(data = as.data.frame(cm$table), aes(x = Reference, y = Prediction, fill = Freq)) +
  geom_tile() + 
...
rf_model_optimized <- caret::train(Stage ~ ., data = trainData, method = "rf",
...
confusionMatrix(rf_predictions_optimized, testData$Stage)
cm <- confusionMatrix(rf_predictions_optimized, testData$Stage)
cm_table <- as.table(cm$table)
accuracy <- sum(diag(cm$table)) / sum(cm$table)
ggplot(data = as.data.frame(cm$table), aes(x = Reference, y = Prediction, fill = Freq)) +
  geom_tile() + 
...
#   ┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛
svm_model <- svm(Stage ~ ., data=trainData, kernel="radial", gamma = 0.01, cost = 0.1)
svm_predictions <- predict(svm_model, testData)
#   ┌──────────────────────────────────────────────────────────────────────────────┐
#   │                         7.1. Model evaluation (SVM)                          │
#   └──────────────────────────────────────────────────────────────────────────────┘
confusionMatrix(svm_predictions, testData$Stage)
cm <- confusionMatrix(svm_predictions, testData$Stage)
cm_table <- as.table(cm$table)
accuracy <- sum(diag(cm$table)) / sum(cm$table)
ggplot(data = as.data.frame(cm$table), aes(x = Reference, y = Prediction, fill = Freq)) +
  geom_tile() + 
...
svm_model_optimized <- caret::train(Stage ~ ., data=trainData, method="svmRadial",
...
confusionMatrix(svm_predictions_optimized, testData$Stage)
cm <- confusionMatrix(svm_predictions_optimized, testData$Stage)
cm_table <- as.table(cm$table)
accuracy <- sum(diag(cm$table)) / sum(cm$table)
ggplot(data = as.data.frame(cm$table), aes(x = Reference, y = Prediction, fill = Freq)) +
  geom_tile() + 
